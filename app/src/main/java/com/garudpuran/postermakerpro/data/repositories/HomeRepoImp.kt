...
class HomeRepoImp(private val database: FirebaseFirestore,
    ...
    override suspend fun getAllFeedItems(): List<FeedItem> = suspendCoroutine { continuation ->
    ...
    override suspend fun getAllTrendingStories(): List<TrendingStoriesItemModel> =
        suspendCoroutine { continuation ->
            val db = database.collection(FirebaseStorageConstants.MAIN_TRENDING_STORIES_NODE)
            try {
                ...
                db.get().addOnSuccessListener {
                    ...
                    Log.d("trending_list",list.toString())
                    continuation.resume(list)
                }
            } catch (e: FirebaseFirestoreException) {
                continuation.resume(emptyList())
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                for (doc in categoryQuerySnapshot.documents) {
                    ...
                    val subcollection = db.document(data?.Id!!)
    ...
    override suspend fun getAllPosts(catId: String, subCatId: String): List<PostItem> =
        suspendCoroutine { continuation ->
            val db =
                database.collection(FirebaseStorageConstants.MAIN_CATEGORIES_NODE).document(catId)
                    .collection(FirebaseStorageConstants.MAIN_SUB_CATEGORIES_NODE)
                    .document(subCatId)
                    .collection(FirebaseStorageConstants.SUB_CATEGORIES_POSTS_NODE)
            try {
    ...
    override suspend fun getAllRecharges(): List<RechargeItem> = suspendCoroutine { continuation ->
    ...
    override suspend fun getAllSearchItems(): List<SearchModel> = suspendCoroutine { continuation ->
        val db = database.collection(FirebaseStorageConstants.MAIN_SEARCH_NODE)
        try {
            val list = ArrayList<SearchModel>()
            db.get().addOnSuccessListener {
                for (doc in it.documents) {
                    val data = doc.toObject(SearchModel::class.java)
                    list.add(data!!)
                }
                continuation.resume(list)
            }
